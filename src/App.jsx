...
const CK="ss-catalog-v2";const PK="ss-promo-v1";const PYK="ss-payments-v1";const BCK="ss-budget-ctr";const ADK="ss-admin-cred";const OFK="ss-offers-v2";const RFK="ss-referrals-v1";const GCK="ss-giftcards-v1";
...
function generateGiftSVG(code,items,total,from,to){
  const itemsText=items.slice(0,6).map(a=>a.name).join(", ")+(items.length>6?" +"+( items.length-6)+" mas":"");
...
<rect x="20" y="20" width="760" height="410" rx="16" fill="none" stroke="#818cf8" stroke-width="1.5" stroke-dasharray="8 4"/>
<text x="400" y="65" text-anchor="middle" fill="#c7d2fe" font-family="Arial" font-size="14" letter-spacing="4">SOPORTE SONORO</text>
<text x="400" y="110" text-anchor="middle" fill="#fff" font-family="Arial" font-size="32" font-weight="bold">GIFT CARD</text>
<text x="400" y="145" text-anchor="middle" fill="#a5b4fc" font-family="Arial" font-size="16">Vale por instalacion de software de audio</text>
<rect x="280" y="165" width="240" height="45" rx="10" fill="#4f46e5"/>
<text x="400" y="195" text-anchor="middle" fill="#fff" font-family="monospace" font-size="24" font-weight="bold">${code}</text>
<text x="400" y="240" text-anchor="middle" fill="#c7d2fe" font-family="Arial" font-size="13">${itemsText}</text>
<text x="400" y="275" text-anchor="middle" fill="#34d399" font-family="Arial" font-size="28" font-weight="bold">$${total.toLocaleString("es-AR")}</text>
${from?`<text x="400" y="310" text-anchor="middle" fill="#a5b4fc" font-family="Arial" font-size="14">De: ${from}${to?" | Para: "+to:""}</text>`:""}
<line x1="100" y1="340" x2="700" y2="340" stroke="#4338ca" stroke-width="1"/>
<text x="400" y="370" text-anchor="middle" fill="#818cf8" font-family="Arial" font-size="13">Para coordinar tu instalacion contacta a:</text>
<text x="400" y="395" text-anchor="middle" fill="#e0e7ff" font-family="Arial" font-size="15" font-weight="bold">produccionmusicaargentina@gmail.com</text>
<text x="400" y="420" text-anchor="middle" fill="#6366f1" font-family="Arial" font-size="11">Presenta este codigo al contactarnos</text>
</svg>`;
}
...
const CATS=["Todos","Synth","FX","FX / Bundle","DAW","Librerias","Servicio","Utility"];
...
export default function App(){
  ...
  if(!mode)return(<div style={{fontFamily:"'Inter',system-ui,sans-serif",maxWidth:480,margin:"0 auto",padding:"40px 20px",textAlign:"center"}}>
    ...
    <p style={{color:"#888",fontSize:14,marginBottom:28}}>Instalacion de plugins y software de audio</p>
    <button style={{...bP,width:"100%",padding:"14px",fontSize:15,marginBottom:12}} onClick={()=>setMode("login")}>Administrador</button>
    <div style={{color:"#bbb",fontSize:13,margin:"14px 0"}}>- o -</div>
    ...
    <div style={{borderTop:"1px solid #eee",paddingTop:16}}>
      ...
      <div style={{...crd,textAlign:"left",marginTop:10}}><div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><span style={{fontWeight:800,color:"#4f46e5"}}>#{trackResult.budgetNumber}</span><StB status={trackResult.status||"pending"}/></div><div style={{fontSize:13,color:"#666",marginTop:6}}>Total: <strong>${(trackResult.total||0).toLocaleString("es-AR")}</strong></div>{trackResult.status==="accepted"&&<div style={{fontSize:13,color:"#059669",marginTop:4,fontWeight:600}}>Tu presupuesto fue aceptado! Te contactamos pronto.</div>}{trackResult.status==="installed"&&<div style={{fontSize:13,color:"#1e40af",marginTop:4,fontWeight:600}}>Instalacion completada!</div>}</div>)}
    </div>
  </div>);
  if(mode==="login")return<AdminLogin onAuth={()=>setMode("admin")} onBack={()=>setMode(null)}/>;
...
function AdminLogin({onAuth,onBack}){
  ...
  return(<div style={{fontFamily:"'Inter',system-ui,sans-serif",maxWidth:380,margin:"0 auto",padding:"60px 20px"}}>
    <button onClick={onBack} style={{background:"none",border:"none",fontSize:16,cursor:"pointer",color:"#999",marginBottom:16}}>Volver</button>
    <div style={{...sec,padding:28}}><div style={{fontSize:14,fontWeight:800,color:"#4f46e5",marginBottom:4,letterSpacing:1}}>SOPORTE SONORO</div>
    <h3 style={{margin:"0 0 20px",fontSize:17}}>{isSetup?"Login":"Crear cuenta"}</h3>
    ...
    <button style={{...bP,width:"100%",marginTop:18,padding:"12px"}} onClick={isSetup?handleLogin:handleSetup}>{isSetup?"Entrar":"Crear"}</button></div>
  </div>);
}
...
function AdminPanel({onBack}){
  ...
  const[copied,setCopied]=useState(false);const[referrals,setReferrals]=useState([]);const fileRef=useRef(null);
  ...
  useEffect(()=>{(async()=>{
    ...
    try{const p=await window.storage.get(RFK);if(p?.value)setReferrals(JSON.parse(p.value));}catch{}
    setLoaded(true);})();},[]);
  ...
  useEffect(()=>{if(loaded)try{window.storage.set(RFK,JSON.stringify(referrals))}catch{}},[referrals,loaded]);
  ...
  function genExp(r){const l=[];l.push("SOPORTE SONORO - #"+(r.budgetNumber||""));l.push("Cliente: "+(r.clientName||"")+(r.isGiftCard?" [GIFT CARD]":""));l.push("Email: "+(r.clientEmail||""));l.push("---");const pd=(r.selectedApps||[]).filter(a=>!a.isGift);pd.forEach((a,i)=>{l.push((i+1)+". "+a.name+" $"+(a.price||0).toLocaleString("es-AR"));(a.links||[]).forEach(lk=>l.push("   "+lk.url));});if((r.customRequests||[]).length){l.push("--- SOLICITUDES ---");r.customRequests.forEach(cr=>l.push("* "+cr.name+(cr.note?" - "+cr.note:"")));}l.push("---\nTOTAL: $"+(r.total||0).toLocaleString("es-AR"));return l.join("\n");}
  ...
  return(<div style={{fontFamily:"'Inter',system-ui,sans-serif",maxWidth:780,margin:"0 auto",padding:"12px 16px 60px"}}>
    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:16,flexWrap:"wrap"}}>
      <button onClick={onBack} style={{background:"#fee2e2",border:"none",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:12,fontWeight:600,color:"#991b1b"}}>Salir</button>
      <div style={{fontSize:15,fontWeight:800,flex:"1 1 60px",color:"#4f46e5",letterSpacing:1}}>SOPORTE SONORO</div>
      {["catalog","promo","offers","session","share","payment","responses","referrals","stats"].map(t=>(<button key={t} style={tB(tab===t)} onClick={()=>{setTab(t);setShowExp(false);setViewR(null);if(t==="responses"||t==="stats")loadResps();}}>{{catalog:"Catalogo",promo:"Promo",offers:"Ofertas",session:"Trabajo",share:"Compartir",payment:"Pagos",responses:"Presup.",referrals:"Referidos",stats:"Stats"}[t]}</button>))}
    </div>

    {tab==="catalog"&&<div>
      <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}><input style={{...inp,flex:1,minWidth:120}} placeholder="Buscar..." value={search} onChange={e=>setSearch(e.target.value)}/><button style={bP} onClick={()=>{setEditApp(null);setShowForm(true);}}>+ App</button><button style={bO} onClick={()=>fileRef.current?.click()}>Excel</button><input ref={fileRef} type="file" accept=".xlsx,.xls,.csv" hidden onChange={handleImport}/></div>
      <div style={{fontSize:12,color:"#888",marginBottom:8}}>{apps.length} items</div>
      {apps.filter(a=>a.name.toLowerCase().includes(search.toLowerCase())||(a.category||"").toLowerCase().includes(search.toLowerCase())).map(a=>(<div key={a.id} style={{...crd,borderLeft:a.giftEligible?"4px solid #f59e0b":undefined}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><span style={{fontWeight:700,fontSize:14,flex:1}}>{a.name}</span>{a.giftEligible&&<GiftE/>}<OsB os={a.os}/>{a.category&&<span style={{fontSize:11,color:"#777",background:"#f3f4f6",padding:"2px 7px",borderRadius:4}}>{a.category}</span>}<span style={{fontWeight:700,color:"#4f46e5",fontSize:14}}>${(a.price||0).toLocaleString("es-AR")}</span></div>
        <div style={{marginTop:6,display:"flex",gap:6,flexWrap:"wrap"}}><button style={{...bS,color:a.giftEligible?"#f59e0b":"#aaa",borderColor:a.giftEligible?"#f59e0b":"#ddd"}} onClick={()=>toggleGE(a.id)}>Regalo</button><button style={bS} onClick={()=>{setEditApp(a);setShowForm(true);}}>Editar</button><button style={{background:"none",border:"none",color:"#ccc",cursor:"pointer"}} onClick={()=>delApp(a.id)}>x</button></div>
      </div>))}
    </div>}

    {tab==="promo"&&<div style={sec}><h3 style={{margin:"0 0 12px"}}>Promo</h3><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}><span style={{fontSize:14,fontWeight:600}}>Cada</span><input type="number" min={2} max={20} value={pEvery} onChange={e=>setPEvery(Math.max(2,Number(e.target.value)||3))} style={{...inp,width:55,textAlign:"center",padding:"4px 8px"}}/><span style={{fontSize:14,fontWeight:600}}>apps = 1 gratis</span></div>
      {[["cheapest","Auto: mas barato gratis"],["pool","Lista curada"]].map(([v,t])=>(<label key={v} style={{display:"flex",alignItems:"center",gap:10,padding:12,borderRadius:8,border:pMode===v?"2px solid #4f46e5":"2px solid #e2e2e8",cursor:"pointer",background:pMode===v?"#eef2ff":"#fff",marginBottom:8}} onClick={()=>setPMode(v)}><input type="radio" checked={pMode===v} onChange={()=>{}}/><span style={{fontWeight:700,fontSize:14}}>{t}</span></label>))}</div>}

    {tab==="offers"&&<div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><h3 style={{margin:0}}>Ofertas</h3><button style={bP} onClick={()=>{setEditOffer(null);setShowOfferForm(true);}}>+ Crear</button></div>
      {offers.length===0&&<div style={{textAlign:"center",color:"#bbb",padding:40}}>Sin ofertas</div>}
      {offers.map(o=>{const isAct=isOfferActive(o);const disc=o.originalPrice>0&&o.price>0?Math.round((1-o.price/o.originalPrice)*100):0;return(<div key={o.id} style={{...crd,borderLeft:o.isSuper?"4px solid #ef4444":"4px solid #f59e0b",opacity:isAct?1:.6}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>{o.isSuper&&<span style={{background:"#ef4444",color:"#fff",padding:"2px 8px",borderRadius:4,fontSize:10,fontWeight:800}}>SUPER</span>}<span style={{fontWeight:800,fontSize:15,flex:1}}>{o.name}</span><span style={{fontWeight:800,color:"#ef4444"}}>${(o.price||0).toLocaleString("es-AR")}</span>{disc>0&&<span style={{background:"#dcfce7",color:"#166534",padding:"2px 8px",borderRadius:4,fontSize:11,fontWeight:700}}>-{disc}%</span>}</div>
        <div style={{fontSize:12,color:"#888",marginTop:4}}>{(o.plugins||[]).length} plugins {o.endDate&&<span> | <Countdown endDate={o.endDate}/></span>}</div>
        <div style={{display:"flex",gap:8,marginTop:8}}><button style={bS} onClick={()=>{setEditOffer(o);setShowOfferForm(true);}}>Editar</button><button style={{...bS,color:o.active?"#ef4444":"#059669"}} onClick={()=>setOffers(offers.map(x=>x.id===o.id?{...x,active:!x.active}:x))}>{o.active?"Desactivar":"Activar"}</button><button style={{background:"none",border:"none",color:"#ccc",cursor:"pointer"}} onClick={()=>setOffers(offers.filter(x=>x.id!==o.id))}>x</button></div>
      </div>);})}
    ...
    {tab==="session"&&!showExp&&<div>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:8}}><span style={{fontSize:13,color:"#666"}}>{sel.size} sel</span>{maxG>0&&<span style={{fontSize:13,color:"#b45309",fontWeight:600}}>Regalos: {giftIds.size}/{maxG}</span>}<div style={{flex:1}}/>{sel.size>0&&<button style={bG} onClick={()=>setShowExp(true)}>Generar</button>}</div>
      {filt.map(a=>{const is=sel.has(a.id);const ig=giftIds.has(a.id);return(<div key={a.id} style={ig?crdG:is?crdSel:crd}><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><input type="checkbox" checked={is} onChange={()=>toggleS(a.id)} style={{width:18,height:18,accentColor:"#4f46e5"}}/><span style={{fontWeight:700,fontSize:14,flex:1,cursor:"pointer"}} onClick={()=>toggleS(a.id)}>{a.name}</span>{ig&&<Gift/>}<OsB os={a.os}/><span style={{fontWeight:700,color:ig?"#16a34a":"#4f46e5",fontSize:14,textDecoration:ig?"line-through":"none"}}>${(a.price||0).toLocaleString("es-AR")}</span>{is&&<button onClick={()=>toggleG(a.id)} style={{background:ig?"#fbbf24":"#f3f4f6",border:"none",borderRadius:6,padding:"3px 8px",cursor:"pointer"}}>R</button>}</div></div>);})}
      {sel.size>0&&<div style={{position:"sticky",bottom:0,background:"#f0f0ff",borderRadius:12,padding:"12px 16px",marginTop:14,borderTop:"2px solid #4f46e5",display:"flex",alignItems:"center",gap:10}}><span style={{fontWeight:700,fontSize:15,flex:1}}>Total: ${totP.toLocaleString("es-AR")}</span><button style={bG} onClick={()=>setShowExp(true)}>Generar</button></div>}
    </div>}
    ...
    {tab==="share"&&<div style={sec}><h3 style={{margin:"0 0 10px"}}>Compartir</h3>{!sesCode?<button style={{...bP,padding:"12px 28px"}} onClick={createSes}>Generar codigo</button>:(<div><div style={{fontSize:36,fontWeight:800,letterSpacing:6,color:"#4f46e5",textAlign:"center",padding:"16px 0",background:"#eef2ff",borderRadius:10,marginBottom:12}}>{sesCode}</div><div style={{display:"flex",gap:8}}><button style={bO} onClick={()=>doCopy(sesCode)}>{copied?"OK":"Copiar"}</button><button style={bO} onClick={()=>setSesCode("")}>Otro</button></div></div>)}</div>}
    ...
    {tab==="responses"&&!viewR&&<div><div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}><h3 style={{margin:0}}>Presupuestos</h3><button style={bS} onClick={loadResps}>Actualizar</button></div>
      ...
      {resps.map((r,i)=>(<div key={i} style={{...crd,padding:16,cursor:"pointer"}} onClick={()=>setViewR(r)}><div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><span style={{fontWeight:800,fontSize:14,color:"#4f46e5"}}>#{r.budgetNumber||""}</span>{r.isGiftCard&&<span style={{background:"#a855f7",color:"#fff",padding:"2px 8px",borderRadius:4,fontSize:10,fontWeight:700}}>GIFT</span>}<span style={{fontWeight:700,flex:1}}>{r.clientName||""}</span><StB status={r.status||"pending"}/></div><div style={{fontSize:13,color:"#888",marginTop:4}}>${(r.total||0).toLocaleString("es-AR")} {r.referralCode&&<span style={{color:"#7c3aed"}}>REF:{r.referralCode}</span>}</div></div>))}
    </div>}
    {tab==="responses"&&viewR&&<div><button style={{...bS,marginBottom:14}} onClick={()=>setViewR(null)}>Volver</button><div style={sec}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,flexWrap:"wrap"}}><span style={{fontWeight:800,fontSize:18,color:"#4f46e5"}}>#{viewR.budgetNumber}</span>{viewR.isGiftCard&&<span style={{background:"#a855f7",color:"#fff",padding:"3px 10px",borderRadius:6,fontSize:12,fontWeight:700}}>GIFT CARD</span>}<StB status={viewR.status||"pending"}/></div>
      <div style={{fontSize:14,marginBottom:16,lineHeight:1.8}}>Cliente: <strong>{viewR.clientName}</strong><br/>Email: {viewR.clientEmail}<br/>OS: <OsB os={viewR.osFilter||"both"}/><br/>{viewR.giftTo&&<>Para: <strong>{viewR.giftTo}</strong><br/></>}{viewR.referralCode&&<>Referido: <strong>{viewR.referralCode}</strong><br/></>}</div>
      {(viewR.selectedApps||[]).map((a,j)=><div key={j} style={{padding:"8px 0",borderBottom:"1px solid #eee",display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><span style={{flex:1,fontWeight:600}}>{a.name}</span>{a.isGift&&<Gift/>}<span style={{fontWeight:700,color:a.isGift?"#16a34a":"#4f46e5"}}>{a.isGift?"$0":"$"+(a.price||0).toLocaleString("es-AR")}</span></div>)}
      {(viewR.customRequests||[]).length>0&&<div style={{marginTop:12,background:"#f5f3ff",borderRadius:8,padding:12,border:"1px solid #c4b5fd"}}><div style={{fontWeight:700,fontSize:13,color:"#7c3aed",marginBottom:6}}>Solicitudes</div>{viewR.customRequests.map((r,i)=><div key={i} style={{fontSize:14}}><strong>{r.name}</strong>{r.note?" - "+r.note:""}</div>)}</div>}
      <div style={{fontWeight:800,fontSize:18,color:"#4f46e5",margin:"16px 0"}}>Total: ${(viewR.total||0).toLocaleString("es-AR")}</div>
      <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:14}}>
        ...
        <button style={bO} onClick={()=>doCopy(genExp(viewR))}>{copied?"OK":"Copiar"}</button><button style={bO} onClick={()=>printText("SS",genExp(viewR))}>Imprimir</button>
      </div>
    </div></div>}

    {tab==="referrals"&&<div><h3 style={{margin:"0 0 14px"}}>Referidos</h3>
      <p style={{fontSize:13,color:"#888",marginBottom:14}}>Los clientes reciben un codigo al enviar presupuesto. Si alguien usa ese codigo, aparece aca.</p>
      {referrals.length===0&&<div style={{textAlign:"center",color:"#bbb",padding:40}}>Sin referidos aun</div>}
      {referrals.map((r,i)=>(<div key={i} style={crd}><div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><span style={{fontWeight:800,color:"#7c3aed"}}>REF-{r.code}</span><span style={{flex:1}}>{r.referrerName}</span><span style={{fontSize:13,color:"#888"}}>{r.usedBy?.length||0} usos</span></div></div>))}
    </div>}

    {tab==="stats"&&<div><h3 style={{margin:"0 0 16px"}}>Estadisticas</h3>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:20}}>
        {[["Facturado","$"+totalRevenue.toLocaleString("es-AR"),"#4f46e5"],["Aceptados",""+accepted.length,"#059669"],["Totales",""+resps.length,"#f59e0b"],["Gift Cards",""+resps.filter(r=>r.isGiftCard).length,"#a855f7"]].map(([l,v,c])=>(<div key={l} style={{background:"#fff",borderRadius:10,padding:"16px 14px",border:"1.5px solid #e8e8ee",textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:c}}>{v}</div><div style={{fontSize:12,color:"#888",marginTop:4}}>{l}</div></div>))}
      </div>
      {monthlyData.length>0&&<div style={{...sec,marginBottom:16}}><div style={{fontWeight:700,fontSize:14,marginBottom:12}}>Mensual</div><ResponsiveContainer width="100%" height={200}><BarChart data={monthlyData}><XAxis dataKey="month" tick={{fontSize:11}}/><YAxis tick={{fontSize:11}}/><Tooltip formatter={v=>"$"+v.toLocaleString("es-AR")}/><Bar dataKey="total" fill="#4f46e5" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></div>}
      {catData.length>0&&<div style={sec}><div style={{fontWeight:700,fontSize:14,marginBottom:12}}>Por categoria</div><ResponsiveContainer width="100%" height={200}><PieChart><Pie data={catData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={75} label={({name,percent})=>name+" "+Math.round(percent*100)+"%"} labelLine={false} style={{fontSize:10}}>{catData.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie><Tooltip formatter={v=>"$"+v.toLocaleString("es-AR")}/></PieChart></ResponsiveContainer></div>}
    </div>}
...
function ClientPanel({code,onBack}){
  ...
  const totP=paid.reduce((s,a)=>s+(a.price||0),0)+activeOffers.reduce((s,o)=>s+(o.price||0),0);
  const toggleS=id=>{setSel(p=>{const n=new Set(p);n.has(id)?n.delete(id):n.add(id);return n;});};
  ...
  async function submit(){
    ...
    try{await window.storage.set("ss-resp-"+code+"-"+uid(),JSON.stringify(r),true);
      ...
      if(isGift){const svg=generateGiftSVG(num,selApps,totP,giftFrom.trim(),giftTo.trim());setGiftSvg(svg);}
      setDone(true);}catch{alert("Error");}
  }

  function downloadGift(){if(!giftSvg)return;const blob=new Blob([giftSvg],{type:"image/svg+xml"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="GiftCard-SoporteSonoro-"+budN+".svg";a.click();URL.revokeObjectURL(url);}

  if(!loaded)return<div style={{padding:40,textAlign:"center",fontFamily:"system-ui",color:"#888"}}>Cargando...</div>;

  // STEP 1: OS
  if(!osF)return(<div style={{fontFamily:"'Inter',system-ui,sans-serif",maxWidth:480,margin:"0 auto",padding:"50px 20px",textAlign:"center"}}>
    <button onClick={onBack} style={{background:"none",border:"none",fontSize:14,cursor:"pointer",color:"#999",marginBottom:20,display:"block"}}>Volver</button>
    <div style={{fontSize:16,fontWeight:800,color:"#4f46e5",letterSpacing:1,marginBottom:8}}>SOPORTE SONORO</div>
    <h2 style={{fontSize:20,marginBottom:24}}>Sistema operativo</h2>
    <div style={{display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap"}}>
      <button onClick={()=>setOsF("windows")} style={{padding:"28px 36px",borderRadius:16,border:"3px solid #dbeafe",background:"#fff",cursor:"pointer",fontSize:16,fontWeight:700,minWidth:160}}>Windows</button>
      <button onClick={()=>setOsF("mac")} style={{padding:"28px 36px",borderRadius:16,border:"3px solid #f3e8ff",background:"#fff",cursor:"pointer",fontSize:16,fontWeight:700,minWidth:160}}>macOS</button>
    </div>
  </div>);

  // STEP 1b: mac ver
  if(osF==="mac"&&!macVer)return(<div style={{fontFamily:"'Inter',system-ui,sans-serif",maxWidth:500,margin:"0 auto",padding:"50px 20px",textAlign:"center"}}>
    <button onClick={()=>setOsF(null)} style={{background:"none",border:"none",fontSize:14,cursor:"pointer",color:"#999",marginBottom:16}}>Cambiar</button>
    <h2 style={{fontSize:20,marginBottom:24}}>Version de macOS</h2>
    <div style={{display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap"}}>
      <button onClick={()=>setMacVer("new")} style={{padding:"24px 28px",borderRadius:16,border:"3px solid #d1fae5",background:"#fff",cursor:"pointer",fontWeight:700,minWidth:180,textAlign:"left"}}><div style={{fontWeight:800,fontSize:16}}>Monterey+</div><div style={{color:"#888",fontSize:12,marginTop:4}}>macOS 12, 13, 14, 15</div></button>
      <button onClick={()=>setMacVer("old")} style={{padding:"24px 28px",borderRadius:16,border:"3px solid #fef3c7",background:"#fff",cursor:"pointer",fontWeight:700,minWidth:180,textAlign:"left"}}><div style={{fontWeight:800,fontSize:16}}>Big Sur o anterior</div><div style={{color:"#888",fontSize:12,marginTop:4}}>macOS 11, 10.15...</div></button>
    </div>
  </div>);

  // STEP 2: Propio o regalo
  if(isGift===null)return(<div style={{fontFamily:"'Inter',system-ui,sans-serif",maxWidth:480,margin:"0 auto",padding:"50px 20px",textAlign:"center"}}>
    <button onClick={()=>{if(osF==="mac")setMacVer(null);else setOsF(null);}} style={{background:"none",border:"none",fontSize:14,cursor:"pointer",color:"#999",marginBottom:16}}>Atras</button>
    <div style={{fontSize:16,fontWeight:800,color:"#4f46e5",letterSpacing:1,marginBottom:8}}>SOPORTE SONORO</div>
    <h2 style={{fontSize:20,marginBottom:24}}>Es para vos o para regalar?</h2>
    <div style={{display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap"}}>
      <button onClick={()=>setIsGift(false)} style={{padding:"28px 36px",borderRadius:16,border:"3px solid #d1fae5",background:"#fff",cursor:"pointer",fontSize:16,fontWeight:700,minWidth:170}}>Para mi</button>
      <button onClick={()=>setIsGift(true)} style={{padding:"28px 36px",borderRadius:16,border:"3px solid #e9d5ff",background:"#fff",cursor:"pointer",fontSize:16,fontWeight:700,minWidth:170}}>Para regalar</button>
    </div>
  </div>);
  ...
  return(<div style={{fontFamily:"'Inter',system-ui,sans-serif",maxWidth:560,margin:"0 auto",padding:"30px 20px"}}>
    <div style={{textAlign:"center",marginBottom:20}}><div style={{fontSize:14,fontWeight:800,color:"#4f46e5",letterSpacing:1,marginBottom:4}}>SOPORTE SONORO</div><h2 style={{marginBottom:4,fontSize:20}}>{isGift?"Gift Card generada!":"Presupuesto enviado!"}</h2><div style={{fontSize:22,fontWeight:800,color:"#4f46e5",marginBottom:8}}>#{budN}</div><p style={{color:"#666"}}>Total: <strong>${totP.toLocaleString("es-AR")}</strong></p></div>
    {isGift&&giftSvg&&<div style={{marginBottom:20}}>
      <div dangerouslySetInnerHTML={{__html:giftSvg}} style={{borderRadius:16,overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,.2)",marginBottom:12}}/>
      <button style={{...bP,width:"100%",padding:"12px",background:"#7c3aed"}} onClick={downloadGift}>Descargar Gift Card (SVG)</button>
    </div>}
    <div style={{background:"#f5f3ff",borderRadius:10,padding:14,marginBottom:16,border:"1px solid #c4b5fd",textAlign:"center"}}><div style={{fontSize:13,color:"#666",marginBottom:4}}>Tu codigo de referido:</div><div style={{fontSize:20,fontWeight:800,color:"#7c3aed",letterSpacing:3}}>{myRefCode}</div><div style={{fontSize:12,color:"#888",marginTop:4}}>Comparti este codigo y obtene beneficios</div></div>
    {payL.length>0&&<div style={{background:"#f0fdf4",borderRadius:12,padding:16,border:"1.5px solid #bbf7d0",marginBottom:16}}><div style={{fontWeight:700,fontSize:15,marginBottom:10}}>Medios de pago</div>{payL.filter(p=>p.alias||p.label).map((pm,i)=>(<div key={i} style={{background:"#fff",borderRadius:8,padding:"10px 12px",marginBottom:6,border:"1px solid #d1fae5"}}><div style={{fontWeight:700,fontSize:14,color:"#065f46"}}>{pm.label}</div><div style={{fontSize:13}}>{pm.alias&&<div>Alias: <strong>{pm.alias}</strong></div>}{pm.cbu&&<div>CBU: {pm.cbu}</div>}{pm.titular&&<div>Titular: {pm.titular}</div>}</div></div>))}</div>}
    {!isGift&&<div style={{background:"#eef2ff",borderRadius:12,padding:16,border:"1.5px solid #c7d2fe",marginBottom:16}}><div style={{fontWeight:800,fontSize:15,marginBottom:8,color:"#4f46e5"}}>Prepara tu equipo</div><p style={{fontSize:13,color:"#555",marginBottom:12}}>Descarga AnyDesk:</p>
      <a href={adUrl} target="_blank" rel="noopener noreferrer" style={{display:"block",padding:"12px",background:"#ef4444",color:"#fff",borderRadius:10,fontWeight:700,textAlign:"center",textDecoration:"none",marginBottom:12}}>Descargar AnyDesk</a>
      {!isWin&&<div style={{background:"#fff",borderRadius:8,padding:14,border:"1px solid #c7d2fe",fontSize:13,lineHeight:1.7}}><strong>Permisos macOS:</strong><br/>1. Preferencias - Privacidad y Seguridad<br/>2. Accesibilidad: agregar AnyDesk<br/>3. Grabacion de pantalla: agregar AnyDesk<br/>4. Reiniciar AnyDesk{vidUrl&&<><br/><a href={vidUrl} target="_blank" rel="noopener noreferrer" style={{color:"#4f46e5",fontWeight:600}}>Ver video tutorial</a></>}</div>}
      {isWin&&<div style={{background:"#fff",borderRadius:8,padding:14,border:"1px solid #c7d2fe",fontSize:13,lineHeight:1.7}}><strong>Windows:</strong><br/>1. Instala AnyDesk<br/>2. Anota tu ID de 9 digitos<br/>3. Compartilo cuando coordinemos<br/>4. Acepta la conexion</div>}
    </div>}
  ...
  return(<div style={{fontFamily:"'Inter',system-ui,sans-serif",maxWidth:740,margin:"0 auto",padding:"12px 16px 80px"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}><button onClick={()=>setIsGift(null)} style={{background:"none",border:"none",fontSize:18,cursor:"pointer",color:"#999"}}>{"<"}</button><div style={{fontSize:16,fontWeight:800,flex:1,color:"#4f46e5",letterSpacing:1}}>SOPORTE SONORO</div><OsB os={eOs}/>{isGift&&<span style={{background:"#a855f7",color:"#fff",padding:"2px 8px",borderRadius:4,fontSize:11,fontWeight:700}}>GIFT</span>}</div>
    <div style={{background:"#fffbeb",padding:"10px 14px",borderRadius:8,fontSize:13,marginBottom:14,color:"#92400e",border:"1px solid #fde68a"}}><strong>Promo:</strong> {promo.mode==="cheapest"?"cada "+(promo.every+1)+" apps, la mas barata gratis":"cada "+promo.every+" apps, elegis 1 de regalo"}</div>

    {offers.length>0&&<div style={{marginBottom:16}}><div style={{fontSize:14,fontWeight:700,marginBottom:8,color:"#ef4444"}}>Ofertas</div>{offers.map(o=>{const isSel=selOffers.has(o.id);const disc=o.originalPrice>0&&o.price>0?Math.round((1-o.price/o.originalPrice)*100):0;return(<div key={o.id} style={{...crd,borderColor:isSel?"#ef4444":"#fca5a5",background:isSel?"#fef2f2":o.isSuper?"#fff5f5":"#fff",border:isSel?"2px solid #ef4444":o.isSuper?"3px solid #ef4444":"2px solid #fca5a5"}} onClick={()=>{setSelOffers(p=>{const n=new Set(p);n.has(o.id)?n.delete(o.id):n.add(o.id);return n;});}}>
      <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",cursor:"pointer"}}><input type="checkbox" checked={isSel} onChange={()=>{}}/>{o.isSuper&&<span style={{background:"#ef4444",color:"#fff",padding:"3px 10px",borderRadius:6,fontSize:11,fontWeight:800}}>SUPER OFERTA</span>}<span style={{fontWeight:800,fontSize:o.isSuper?17:15,flex:1,color:"#ef4444"}}>{o.name}</span><span style={{fontWeight:800,color:"#ef4444"}}>${(o.price||0).toLocaleString("es-AR")}</span>{disc>0&&<span style={{background:"#dcfce7",color:"#166534",padding:"2px 8px",borderRadius:4,fontSize:12,fontWeight:800}}>-{disc}%</span>}</div>
      {o.description&&<p style={{fontSize:13,color:"#666",margin:"4px 0 0 26px"}}>{o.description}</p>}{o.endDate&&<div style={{marginLeft:26,marginTop:4}}><Countdown endDate={o.endDate}/></div>}
    </div>);})}</div>}

    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
      <div><label style={{fontSize:12,color:"#666",marginBottom:4,display:"block"}}>Nombre *</label><input style={inp} value={cName} onChange={e=>setCName(e.target.value)}/></div>
      <div><label style={{fontSize:12,color:"#666",marginBottom:4,display:"block"}}>Email *</label><input style={inp} type="email" value={cEmail} onChange={e=>setCEmail(e.target.value)}/></div>
    </div>
    {isGift&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
      <div><label style={{fontSize:12,color:"#666",marginBottom:4,display:"block"}}>De (tu nombre)</label><input style={inp} value={giftFrom} onChange={e=>setGiftFrom(e.target.value)}/></div>
      <div><label style={{fontSize:12,color:"#666",marginBottom:4,display:"block"}}>Para (destinatario)</label><input style={inp} value={giftTo} onChange={e=>setGiftTo(e.target.value)}/></div>
    </div>}
    <div style={{marginBottom:10}}><label style={{fontSize:12,color:"#666",marginBottom:4,display:"block"}}>Codigo de referido (opcional)</label><input style={{...inp,maxWidth:200}} value={refCode} onChange={e=>setRefCode(e.target.value)} placeholder="REF-XXXXXX"/></div>

    <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap",overflowX:"auto"}}>{["Todos",...availCats].filter((v,i,a)=>a.indexOf(v)===i).map(c=><button key={c} style={tB(catFilter===c)} onClick={()=>setCatFilter(c)}>{c}</button>)}</div>
    <input style={{...inp,marginBottom:12}} placeholder="Buscar plugin..." value={search} onChange={e=>setSearch(e.target.value)}/>

    {filt.map(a=>{const is=sel.has(a.id);const ig=autoGift.has(a.id);return(<div key={a.id} style={ig?crdG:is?crdSel:crd}><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><input type="checkbox" checked={is} onChange={()=>toggleS(a.id)} style={{width:18,height:18,accentColor:"#4f46e5"}}/><span style={{fontWeight:700,fontSize:14,flex:1,cursor:"pointer"}} onClick={()=>toggleS(a.id)}>{a.name}</span>{ig&&<Gift/>}<OsB os={a.os}/>{a.category&&<span style={{fontSize:11,color:"#777",background:"#f3f4f6",padding:"2px 7px",borderRadius:4}}>{a.category}</span>}<span style={{fontWeight:700,color:ig?"#16a34a":"#4f46e5",fontSize:14,textDecoration:ig?"line-through":"none"}}>${(a.price||0).toLocaleString("es-AR")}</span></div></div>);})}

    <div style={{marginTop:14,marginBottom:10}}><button style={{...bO,width:"100%",padding:"10px",fontSize:13,borderStyle:"dashed"}} onClick={()=>setShowReqForm(true)}>+ No encontras algo? Pedilo</button></div>
    {showReqForm&&<div style={{...sec,marginBottom:14}}><label style={{fontSize:12,color:"#666",display:"block",marginBottom:4}}>Nombre *</label><input style={inp} value={reqName} onChange={e=>setReqName(e.target.value)} autoFocus/><label style={{fontSize:12,color:"#666",display:"block",marginBottom:4,marginTop:10}}>Nota</label><input style={inp} value={reqNote} onChange={e=>setReqNote(e.target.value)}/><div style={{display:"flex",gap:8,marginTop:12}}><button style={bG} onClick={addReq}>Agregar</button><button style={bO} onClick={()=>{setShowReqForm(false);setReqName("");setReqNote("");}}>X</button></div></div>}
    {customReqs.length>0&&customReqs.map(r=>(<div key={r.id} style={{...crd,borderStyle:"dashed",borderColor:"#c4b5fd",display:"flex",gap:8,alignItems:"center"}}><span style={{flex:1,fontSize:14}}><strong>{r.name}</strong>{r.note?" - "+r.note:""}</span><span style={{fontSize:12,color:"#888"}}>Consultar</span><button onClick={()=>setCustomReqs(p=>p.filter(x=>x.id!==r.id))} style={{background:"none",border:"none",color:"#ccc",cursor:"pointer"}}>x</button></div>))}

    {promo.mode==="pool"&&maxPG>0&&<div style={{background:"#fffbeb",borderRadius:12,padding:16,marginTop:14,border:"1.5px solid #fde68a"}}><div style={{fontWeight:700,fontSize:15,marginBottom:8}}>Elegi {maxPG} regalo{maxPG>1?"s":""}</div>{gPool.filter(a=>a.os===eOs||a.os==="both").map(a=>{const pk=gSel.has(a.id);return(<div key={a.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",marginBottom:6,borderRadius:8,background:pk?"#fef3c7":"#fff",border:pk?"2px solid #f59e0b":"1.5px solid #e2e2e8",cursor:"pointer"}} onClick={()=>togglePG(a.id)}><input type="checkbox" checked={pk} onChange={()=>{}}/><span style={{flex:1,fontWeight:600}}>{a.name}</span><span style={{color:"#16a34a",fontWeight:700}}>GRATIS</span></div>);})}</div>}

    {(sel.size>0||selOffers.size>0||customReqs.length>0)&&<div style={{position:"sticky",bottom:0,background:"#f0f0ff",borderRadius:12,padding:"14px 16px",marginTop:14,borderTop:"2px solid #4f46e5"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><span style={{fontWeight:700,fontSize:16,flex:1}}>Total: ${totP.toLocaleString("es-AR")}{customReqs.length>0?" +"+customReqs.length+" consulta":""}</span></div><button style={{...bG,width:"100%",padding:"12px",fontSize:15,background:isGift?"#7c3aed":"#059669"}} onClick={submit} disabled={!cName.trim()||!cEmail.trim()}>{isGift?"Generar Gift Card":"Enviar presupuesto"}</button>{(!cName.trim()||!cEmail.trim())&&<div style={{fontSize:12,color:"#ef4444",marginTop:6,textAlign:"center"}}>Completa nombre y email</div>}</div>}
  </div>);
}
...
function AppForm({app,onSave,onCancel}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}} onClick={onCancel}><div style={{background:"#fff",borderRadius:16,padding:"24px 20px",width:"100%",maxWidth:500,maxHeight:"90vh",overflow:"auto"}} onClick={e=>e.stopPropagation()}>
    ...
    <label style={lbl}>OS</label><div style={{display:"flex",gap:8}}>{[["both","Ambos"],["windows","Win"],["mac","Mac"]].map(([v,l])=><button key={v} onClick={()=>sO(v)} style={{padding:"7px 14px",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",border:os===v?"2px solid #4f46e5":"2px solid #e2e2e8",background:os===v?"#eef2ff":"#fff",color:os===v?"#4f46e5":"#666"}}>{l}</button>)}</div>
    <label style={lbl}>Precio</label><input style={inp} type="number" value={pr||""} onChange={e=>sP(e.target.value)}/>
    <label style={{...lbl,display:"flex",alignItems:"center",gap:8,cursor:"pointer",marginTop:16}} onClick={()=>sGE(!ge)}><input type="checkbox" checked={ge} onChange={()=>{}}/><span>Elegible regalo</span></label>
    <label style={lbl}>Links</label>{lks.map((l,i)=><div key={i} style={{display:"flex",gap:6,marginBottom:8}}><select value={l.type} onChange={e=>sL(lks.map((x,j)=>j===i?{...x,type:e.target.value}:x))} style={{...inp,width:90}}><option value="magnet">Magnet</option><option value="drive">Drive</option><option value="direct">Direct</option></select><input style={{...inp,flex:1}} value={l.url} onChange={e=>sL(lks.map((x,j)=>j===i?{...x,url:e.target.value}:x))}/>{lks.length>1&&<button onClick={()=>sL(lks.filter((_,j)=>j!==i))} style={{background:"none",border:"none",color:"#ccc",cursor:"pointer"}}>x</button>}</div>)}
    <button onClick={()=>sL([...lks,{type:"magnet",url:""}])} style={{background:"none",border:"none",color:"#4f46e5",cursor:"pointer",fontSize:13}}>+ Link</button>
    <div style={{display:"flex",gap:10,marginTop:20,justifyContent:"flex-end"}}><button onClick={onCancel} style={{padding:"9px 20px",borderRadius:8,border:"1.5px solid #ddd",background:"#fff",cursor:"pointer"}}>X</button><button onClick={()=>{if(!n.trim())return;onSave({id:app?.id,name:n.trim(),os,price:Number(pr)||0,category:cat.trim(),giftEligible:ge,links:lks.filter(l=>l.url.trim())});}} style={bP}>OK</button></div>
  </div></div>);
}
...
function OfferForm({offer,apps,onSave,onCancel}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}} onClick={onCancel}><div style={{background:"#fff",borderRadius:16,padding:"24px 20px",width:"100%",maxWidth:540,maxHeight:"90vh",overflow:"auto"}} onClick={e=>e.stopPropagation()}>
    <div style={{fontWeight:700,fontSize:18,marginBottom:12}}>Oferta</div>
    <label style={{...lbl,marginTop:0}}>Nombre *</label><input style={inp} value={n} onChange={e=>sN(e.target.value)} autoFocus/>
    <label style={lbl}>Descripcion</label><textarea style={{...inp,minHeight:50,resize:"vertical"}} value={desc} onChange={e=>sD(e.target.value)}/>
    <div style={{display:"flex",gap:10}}><div style={{flex:1}}><label style={lbl}>Precio</label><input style={inp} type="number" value={pr||""} onChange={e=>sP(e.target.value)}/></div><div style={{flex:1}}><label style={lbl}>Original</label><input style={inp} type="number" value={oP||""} onChange={e=>sOP(e.target.value)}/></div></div>
    {disc>0&&<div style={{fontSize:13,color:"#059669",fontWeight:700,marginTop:6}}>-{disc}%</div>}
    <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",marginTop:12,padding:"10px 14px",borderRadius:8,background:isSuper?"#fef2f2":"#fafafa",border:isSuper?"2px solid #ef4444":"2px solid #e8e8ee"}} onClick={()=>setIsSuper(!isSuper)}><input type="checkbox" checked={isSuper} onChange={()=>{}}/><span style={{fontWeight:700,color:isSuper?"#ef4444":"#666"}}>SUPER OFERTA</span></label>
    <div style={{background:"#f8fafc",borderRadius:10,padding:14,marginTop:16,border:"1px solid #e2e2e8"}}>
      <div style={{fontWeight:700,fontSize:14,marginBottom:10}}>Programacion</div>
      <div style={{display:"flex",gap:4,marginBottom:12,flexWrap:"wrap"}}>{dayN.map((d,i)=>(<button key={i} onClick={()=>setSchDays(p=>p.includes(i)?p.filter(x=>x!==i):[...p,i])} style={{padding:"6px 12px",borderRadius:6,fontSize:12,fontWeight:600,cursor:"pointer",border:schDays.includes(i)?"2px solid #4f46e5":"2px solid #e2e2e8",background:schDays.includes(i)?"#eef2ff":"#fff",color:schDays.includes(i)?"#4f46e5":"#888"}}>{d}</button>))}</div>
      <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,marginBottom:12}} onClick={()=>setEom(!eom)}><input type="checkbox" checked={eom} onChange={()=>{}}/><span>Solo fin de mes</span></label>
      <div style={{display:"flex",gap:10}}><div style={{flex:1}}><label style={{fontSize:12,color:"#666",display:"block",marginBottom:4}}>Desde</label><input style={inp} type="date" value={startD} onChange={e=>setStartD(e.target.value)}/></div><div style={{flex:1}}><label style={{fontSize:12,color:"#666",display:"block",marginBottom:4}}>Hasta</label><input style={inp} type="date" value={endD} onChange={e=>setEndD(e.target.value)}/></div></div>
    </div>
    <label style={{...lbl,marginTop:16}}>Plugins ({pIds.size})</label>
    <div style={{maxHeight:160,overflow:"auto",border:"1px solid #eee",borderRadius:8,padding:8}}>{apps.map(a=><label key={a.id} style={{display:"flex",gap:8,padding:"3px 0",cursor:"pointer",fontSize:13}}><input type="checkbox" checked={pIds.has(a.id)} onChange={()=>{sPIds(p=>{const nn=new Set(p);nn.has(a.id)?nn.delete(a.id):nn.add(a.id);return nn;});}}/>
    <span style={{flex:1}}>{a.name}</span><span style={{color:"#888"}}>${(a.price||0).toLocaleString("es-AR")}</span></label>)}</div>
    <div style={{display:"flex",gap:10,marginTop:20}}><button onClick={onCancel} style={{padding:"9px 20px",borderRadius:8,border:"1.5px solid #ddd",background:"#fff",cursor:"pointer"}}>X</button><button onClick={()=>{if(!n.trim())return;onSave({id:offer?.id,name:n.trim(),description:desc.trim(),price:Number(pr)||0,originalPrice:Number(oP)||0,plugins:apps.filter(a=>pIds.has(a.id)),active:offer?.active??true,isSuper,scheduleDays:schDays,endOfMonth:eom,startDate:startD||null,endDate:endD||null});}} style={bP}>OK</button></div>
  </div></div>);
}
